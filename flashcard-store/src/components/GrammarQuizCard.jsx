...
/* ---------- GrammarQuizCard ----------
A one-off showcase card for the Grammar Primer's Store preview, styled
after a navy/gold "English Language & Literacy" study-card look — the
same simple ribbon-row template as MathQuizCard. Just a question on the
front, and the answer plus its sentence form on the back.
--------------------------------- */

const NAVY = "#6B1F2E";
const NAVY_DEEP = "#4A1420";
const GOLD = "#B8863F";
const GOLD_LIGHT = "#EDD9C0";
const CREAM = "#FBF3E4";
const INK = "#2B1C1F";
const SKY = "#D9A8A8";

const HEADER_TITLE = { en: "English", km: "ភាសាអង់គ្លេស" };
const HEADER_SUBTITLE = { en: "Language & Literacy", km: "ភាសា និងអក្ខរកម្ម" };
...
const FORM_LABEL = { en: "Form:", km: "រចនាសម្ព័ន្ធ៖" };
const FRONT_TAGLINE = { en: "Master English, express yourself better.", km: "ចេះភាសាអង់គ្លេស បង្ហាញខ្លួនឯងបានប្រសើរជាង។" };
const BACK_TAGLINE = { en: "Practice today, improve tomorrow!", km: "អនុវត្តថ្ងៃនេះ ល្អប្រសើរថ្ងៃស្អែក!" };
const QUESTION = { en: "What is present simple?", km: "តើ present simple គឺជាអ្វី?" };
const ANSWER_TEXT = {
  en: "A tense used for habits, routines, facts, and things that are always or generally true.",
  km: "សម័យកិរិយាស័ព្ទប្រើសម្រាប់ទម្លាប់ ការធ្វើប្រចាំ ការពិត និងអ្វីៗដែលពិតជានិច្ច។",
};
const FORM_PARTS = { en: "Subject + Verb (+s/es for he/she/it) + Object", km: "ប្រធានបទ + កិរិយាស័ព្ទ (+s/es សម្រាប់ he/she/it) + កម្មបទ" };
const EXAMPLE = { en: "Example: She works at a hospital. They play football.", km: "ឧទាហរណ៍៖ She works at a hospital. They play football." };
...
// A quill — the writing half of "language & literacy" — doubles as the
// decorative corner flourish on the header.
function QuillIcon({ className, style }) {
  return (
    <svg viewBox="0 0 24 60" className={className} style={style} fill="currentColor" aria-hidden="true">
      <path d="M12 1c6 6 6 22-1.5 37 4.5-13 3-25-4.5-31Z" opacity="0.85" />
      <line x1="10.5" y1="15" x2="3" y2="43" stroke="currentColor" strokeWidth="1.6" opacity="0.85" />
    </svg>
  );
}
...
function NotesBox() {
  ...
  return (
    <div
      ...
      style={{ border: `2px solid ${NAVY}`, background: CREAM, boxShadow: "0 12px 28px -10px rgba(74,20,32,0.35)" }}
    >
      <span
        ...
        style={{
          ...
          color: CREAM,
          letterSpacing: "1.5px",
          boxShadow: "0 3px 8px rgba(74,20,32,0.4)",
        }}
      >
        {t("custom.noteLabel")}
      </span>
...
export default function GrammarQuizCard({ flipped, onFlip, interactive = true, className }) {
      ...
      >
        ...
        >
          <HeaderBar>
            <div className="flex items-center gap-2.5 min-w-0">
              <BookIcon className="w-6 h-6 shrink-0" />
            ...
            <QuillIcon className="absolute right-5 -bottom-4 w-8 h-14 opacity-90" style={{ color: GOLD }} />
          </HeaderBar>
          ...
          <FooterStrip
            icon={
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="w-full h-full">
                <path d="M9 18h6M10 21h4M12 3a6 6 0 0 0-3.5 10.9c.5.4.8.9.8 1.5v.1h5.4v-.1c0-.6.3-1.1.8-1.5A6 6 0 0 0 12 3Z" />
              </svg>
            }
          >
            {pick(FRONT_TAGLINE)}
        ...
        >
          ...
          <div className="flex-1 min-h-0 flex flex-col px-4 pt-4 pb-3 overflow-hidden">
            ...
            <p className="font-display text-[22px] font-bold text-center tracking-wide my-3" style={{ color: NAVY_DEEP }}>
              S + V + O
            </p>
            <p className="text-[11px] leading-relaxed">{pick(FORM_PARTS)}</p>
